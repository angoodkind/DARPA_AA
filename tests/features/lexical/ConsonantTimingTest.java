...
import features.lexical.ConsonantTiming;
import features.pause.KSE;
import keystroke.KeyStroke;

public class ConsonantTimingTest {
	...
	public void test() {
		...
		Collection<KSE> allKSEs = KSE.parseSessionToKSE(answer.getKeyStrokes());
		for (KSE kse : allKSEs)
			if (kse.isKeyPress())
				keyPressKSEs.add(kse);
	    
	    ConsonantTiming ct = new ConsonantTiming();
	    long commonTime = ct.getCommonConsonantTiming(keyPressKSEs);
	    long rareTime = ct.getRareConsonantTiming(keyPressKSEs);
	    double commonRatio = ct.getCommonConsonantTimingRatio(keyPressKSEs);
	    double rareRatio = ct.getRareConsonantTimingRatio(keyPressKSEs);
	    double commonRareRatio = ct.getCommonToRareConsonantTimingRatio(keyPressKSEs);
	    
	    assertEquals(17,commonTime);
	    assertEquals(20,rareTime);
	    assertEquals(0.8947368421052632,commonRatio,0.001);
	    assertEquals(1.0526315789473684,rareRatio,0.001);
	    assertEquals(0.85,commonRareRatio,0.001);
	}
