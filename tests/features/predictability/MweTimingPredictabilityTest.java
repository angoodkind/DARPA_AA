...
import features.predictability.MweTimingPredictability;
import keystroke.KeyStroke;

public class MweTimingPredictabilityTest {
...
/*---------------------------------------*/
	    MweTimingPredictability mtp = new MweTimingPredictability();
	    mtp.initializeFeatureMap();
	    mtp.generateMweTimingData(answer,0);
	    
//	    for (String s : mtp.getPauseMapBigram().keySet())
//	    	System.out.println(s+" "+mtp.getPauseMapBigram().get(s));
	    
	    ArrayList<Double> startExpected = new ArrayList<Double>(Arrays.asList(0.0017230821367866485, 0.0021816358277728983));
	    ArrayList<Double> endExpected = new ArrayList<Double>(Arrays.asList(0.0016362268708296737));
	    ArrayList<Double> midExpected = new ArrayList<Double>(Arrays.asList(0.0016362268708296737, 0.0013799718743154215));
	    ArrayList<Double> outExpected = new ArrayList<Double>();
	    assertEquals(startExpected,mtp.getPauseMapBigram().get("StartOfMwe"));
	    assertEquals(endExpected,mtp.getPauseMapBigram().get("EndOfMwe"));
	    assertEquals(midExpected,mtp.getPauseMapBigram().get("MiddleOfMwe"));
	    assertEquals(outExpected,mtp.getPauseMapBigram().get("OutsideOfMwe"));
	}
