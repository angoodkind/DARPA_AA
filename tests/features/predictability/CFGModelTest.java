...
public class CFGModelTest {
	...
	@Test
	public void RetrieveSyntacticModelTest() throws InvalidFormatException, IOException {
...
/*---------------------------------------*/
		...
		POS_Extractor posExtractor = new POS_Extractor();
	    String answerCharStream = answer.getCharStream();
	    String[] actualWordTokens = tokenize.runTokenizer(answerCharStream);
	    String[] actualPosTokens = posExtractor.createPOSTags(actualWordTokens);
	    String[] expectedWordTokens = new String[]{"she","looked","up","tehhe","world","record","."};
	    String[] expectedPosTokens = new String[]{"PRP","VBD","IN","DT","NN","NN","."};
	   
	    assertArrayEquals(expectedWordTokens,actualWordTokens);
	    assertArrayEquals(expectedPosTokens,actualPosTokens);
	}

	@SuppressWarnings("deprecation")
	@Test
	public void RetrieveSyntacticPredictabilityTest() throws InvalidFormatException, IOException {
...
/*---------------------------------------*/
	    ...
	    Tokenize tokenize = new Tokenize();
		POS_Extractor posExtractor = new POS_Extractor();
	    BigramModel cfgBigramModel = BigramModel.readFromFile("S1BigramCFG.data");
	    String answerCharStream = answer.getCharStream();
	    String[] actualWordTokens = tokenize.runTokenizer(answerCharStream);
	    String[] actualPosTokens = posExtractor.createPOSTags(actualWordTokens);
...
//	    assertArrayEquals(expectedPosTokens,actualPosTokens);
	    ...
	    for (int i = 0; i < actualPosTokens.length-1; i++) {
	    	String gram1 = actualPosTokens[i];
	    	String gram2 = actualPosTokens[i+1];
	    	Bigram bigram = new Bigram(gram1,gram2);
	    	System.out.println(gram1+" "+gram2);
	    	System.out.println(cfgBigramModel.getBigramCount(bigram));
	    	System.out.println(cfgBigramModel.getBigramProbability(bigram));
	    }
	}
}
