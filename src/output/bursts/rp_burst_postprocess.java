...
public class rp_burst_postprocess {
...
public static LinkedList<Feature>  extract (List<Feature> output_list, ArrayList<Burst> BurstArray) {
			...
			switch (feature.getFeatureName()) {
				...
				case "Lexical_Density": {
					...
					for (Burst b: BurstArray) {
						...
						BurstType_RP rpb = (BurstType_RP)b;
						
						Double val = (Double)lexDensities[++counter];
						Double result = (val.doubleValue() * 1000) / (double) rpb.burstTime();
						normalized.add(result);
						Double result2 = (val.doubleValue() * 1000) / (double) rpb.GetPauseBefore();
						normalized2.add(result2);
						Double result3 = (val.doubleValue() * 1000) / (double) rpb.GetPauseAfter();
		...
		for (Burst b: BurstArray) {
			...
			word_count_ratio.add((double)b.burstWords() * 1000 / (double) rpb.burstTime());
			word_count_ratio_pause_before.add((double)b.burstWords() * 1000 / (double) rpb.GetPauseBefore());
			word_count_ratio_pause_after.add((double)b.burstWords() * 1000 / (double) rpb.GetPauseAfter());
		}
		
		output.add(new Feature("rp_burst_word_count_ratio", word_count_ratio));
		output.add(new Feature("rp_burst_word_count_ratio_Before_Pause", word_count_ratio_pause_before));
		output.add(new Feature("rp_burst_word_count_ratio_After_Pause", word_count_ratio_pause_after));
