 ...
 */
public class RowFingerTransitionSpeed implements ExtractionModule {
	...
	public RowFingerTransitionSpeed() {
		searchSpace.clear();
		featureMap.clear();
		generateSearchSpace();
	}
	...
	public void generateSearchSpace() {
		for (String finger : fingerNames)
			for (String row : rowNames)
				searchSpace.add(row+"_TO_"+finger);
	}
...
//	    for (Feature f : output) System.out.println(f.toTemplate());
	...
	public String getTransitionType(LinkedList<KSE> buffer) {
		...
		String keystroke2Category = buffer.getLast().kseGetFinger();
...
//				(buffer.getLast().getWhen() - buffer.getFirst().getWhen()));
	...
	public String getName() {
		return "Row_Finger_Transition_Speed";
	}
