...
/**
 * @author agoodkind
 * Transition times between each hand to row
 */
public class HandToRowSpeed implements ExtractionModule {
...
//								System.out.println(kse.getKeyChar()+"  "+kse.getRow()+"  "+kse.getHand());
//								System.out.println(kse.getKeyChar()+"  "+kse.getM_pauseMs());
								LinkedList<Long> list = nestedHandMap.get(kse.kseGetRow()).get(kse.kseGetHand());
								list.add(kse.getM_pauseMs());	
//								System.out.println(nestedHandMap);
							}
						}
					}
				}
		...
		CanonicalFinger.addNormedToOutput(rowNames, output, nestedHandMap, "Finger_Speed");
//	    for (Feature f : output) System.out.println(f.toTemplate());
	...
	public String getTransitionType(LinkedList<KSE> buffer) {
		...
		String keystroke2Category = buffer.getLast().kseGetRow();
