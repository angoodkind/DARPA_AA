...
/**
 * @author agoodkind
 * Transition times between each hand-row to hand-row combination
 */
public class RowHandToRowHandSpeed implements ExtractionModule {
		...
		for (String hand : handNames)
			for (String row : rowNames)
				for (String hand2 : handNames)
					for (String row2 : rowNames)
						searchSpace.add(row+"_"+hand+"_TO_"+row2+"_"+hand2);
		}
...
//	    for (Feature f : output) System.out.println(f.toTemplate());
	...
	public String getTransitionType(LinkedList<KSE> buffer) {
		String keystroke1Category = buffer.getFirst().kseGetRow()+"_"+buffer.getFirst().kseGetHand();
		String keystroke2Category = buffer.getLast().kseGetRow()+"_"+buffer.getLast().kseGetHand();
...
//				(buffer.getLast().getWhen() - buffer.getFirst().getWhen()));
	...
	public String getName() {
		return "Row_Hand_To_Row_Hand_Speed";
	}
