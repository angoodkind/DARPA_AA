...
/**
 * @author agoodkind
 * Transition times between each hand-finger to row transition
 */
public class HandFingerToRowTransitionSpeed implements ExtractionModule {
	...
	public HandFingerToRowTransitionSpeed() {
		searchSpace.clear();
		featureMap.clear();
		generateSearchSpace();
	}
	...
	public void generateSearchSpace() {
		for (String hand : handNames)
			for (String finger : fingerNames)
				for (String row : rowNames)
					searchSpace.add(hand+"_"+finger+"_TO_"+row);
		// add SearchSpaces for space and shift
		...
		for (String finger : fingerNames2)
			for (String row : rowNames2)
				searchSpace.add("None_"+finger+"_TO_"+row);
	}
