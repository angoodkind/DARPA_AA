...
/**
 * @author agoodkind
 * Transition times between each 
 */
public class FingerRowTransitionSpeed implements ExtractionModule {
	...
	public FingerRowTransitionSpeed() {
		searchSpace.clear();
		featureMap.clear();
		generateSearchSpace();
	}
	...
	public void generateSearchSpace() {
		for (String finger : fingerNames)
			for (String row : rowNames)
				searchSpace.add(finger+"_TO_"+row);
	}
...
//	    for (Feature f : output) System.out.println(f.toTemplate());
	...
	public String getTransitionType(LinkedList<KSE> buffer) {
		String keystroke1Category = buffer.getFirst().kseGetFinger();
...
//				(buffer.getLast().getWhen() - buffer.getFirst().getWhen()));
	...
	public String getName() {
		return "Finger_Row_Transition_Speed";
	}
