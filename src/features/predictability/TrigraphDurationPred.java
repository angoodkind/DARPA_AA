 ...
 * 
 */
public class TrigraphDurationPred extends Predictability implements ExtractionModule {
	...
	private HashMap<String, LinkedList<String>> featureMap = new HashMap<String, LinkedList<String>>(10000);

	private static final boolean LOG_TIME = false;
	public static final boolean DEBUG = false;
	public static final boolean WRITE_FILE = false;
	public static final String csv_file = "TriHold.csv";

	/**
	 * Initialize the module.
	 */
	public TrigraphDurationPred() {
			...
			for (Integer key1 : keySet)
				for (Integer key2 : keySet)
					featureMap.put((KeyStroke.vkCodetoString(key0)+"_"+KeyStroke.vkCodetoString(key1)
							+"_"+KeyStroke.vkCodetoString(key2)),new LinkedList<String>());
	}
...
//						triDuration = Math.log(triDuration);
					...
					if (featureMap.containsKey(trigraph)) {
						double feature = triDuration;
						if (!(Double.isNaN(feature) || Double.isInfinite(feature))) {
							DecimalFormat df = new DecimalFormat("#.#");
							df.setMaximumFractionDigits(8);
							featureMap.get(trigraph).add(df.format(feature));
						}
					}
				}
			}			
		...
		for (String s : featureMap.keySet()) {
			output.add(new Feature("Tri_Pred_"+s, featureMap.get(s)));
		}
		return output;
	}
