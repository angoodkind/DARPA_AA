 ...
 * @author Patrick
 */
public class TrigraphDuration implements ExtractionModule {

	private Set<String> trigraphs;
	private HashMap<String, LinkedList<Long>> featureMap = new HashMap<String, LinkedList<Long>>(1800);
	...
	public Collection<Feature> extract(DataNode data) {
			...
			/////////////////Main Logic///////////////////////
				...
				if (buffer.size() > 6) {
					...
					if (trigraphs.contains(stringBuffer) && isNormalTrigraph(buffer))
							featureMap.get(stringBuffer).add(buffer.get(4).getWhen() - buffer.getFirst().getWhen());
				}
			/////////////////////////////////////////////////
			}
		}
		...
		for (String s : trigraphs)
			output.add(new Feature("TN_" + s, featureMap.get(s)));
		return output;
	}
