...
public class P_PR_BURST  implements ExtractionModule {
	...
	public Collection<Feature> extract(DataNode data) {
		//CurrentDataNode = data;
				...
				for (Answer a: data) {
					...
					delim_state = state.INITIAL; // Use if you do no assume pause at the start
					...
					for(endIndex = startIndex + 1; endIndex < key_events.size()-1; endIndex++) {
							...
							if (previous_key_event.isKeyRelease()) {//Check for a preceding key release to identify a possible 'pause' occurence
								...
								if (pauseDelimiter.isDelimiter(previous_key_event, current_key_event)) {//If a pause is detected
									...
									switch (delim_state) {
										...
										case SECOND_P: { // If you encounter a third PR....
											System.out.println("PAUSE AT: " + endIndex + " REVISION DEPTH: " + revision_count);
											if (revision_count >= min_revision_count && PR_char_count <= pr_dist) { // Check if the previous (second) P can be associated with a revision
												r2_start_index = revision_start_index;
												r2_end_index = revision_end_index;
												add_clip = true;
												delim_state = state.FIRST_P;
											}
											else { // If not, then assign previous P as P1 and current P as P2
						...
						if (add_clip) {
							...
							//StartEnd clip = new StartEnd(start_pause_index, r2_end_index); // This includes R2 revision end index.
							...
							if (delim_state == state.FIRST_P && pause_index > end_pause_index) {
				...
				output.add(new Feature("PR_PreBurst", preBurstKeyDensity1));
				output.add(new Feature("PR_PreBurst_Alpha", preBurstKeyDensity2));
	...
	private static class CreateFileOnce {
		...
		public static String GetAbsoluteFilePath() {
			if (write_switch) {
				...
				java.io.File output_dir = new java.io.File(workingDir,"BurstStats/P-R");
				output_dir.mkdirs();
				...
				output_file_path = new java.io.File(output_dir.getAbsolutePath(), "PR_BurstsPerAnswer" + now + ".csv").toString();
