...
public class AppWindowScriptAG {
...
//		for (int kt = 0; kt < collectionNumberArray.length; kt++) {
			collectionName = "collection1"; // + Integer.toString(collectionNumberArray[kt]);
			DbConfig.setCollectionName(collectionName);
//			beginningOutputDirectory = Integer.toString(collectionNumberArray[kt] ); 
...
//				currentOutputDirectory = beginningOutputDirectory + "\\P=" + Integer.toString(pauseLength) + "\\template"; 
				...
				for (int i = 0; i < secondsIntervalArray.length; i++) {
					...
					currentOutputDirectory = "TV_Test_AG"+ Integer.toString(currentSliceSize) + "sec"; 
